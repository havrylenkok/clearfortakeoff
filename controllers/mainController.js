        ...
        db.getInfo(req.body.inputVal1, function (err1, res1) {
            if (!err1) {
                db.getInfo(req.body.inputVal2, function (err2, res2) {
                    if (!err2) {
                        metarApi(res1[0].icao, 0, res1[0].ils, res1[0].cource,
                            res2[0].icao, 10, res2[0].ils, res2[0].cource, function (res3) {
                                ...
                                parseFlight(req.body.inputVal2, req.body.inputVal1, new Date().toJSON().slice(0, 10), function (fInfo) {
                                    ...
                                    res.send({
                                        flight_percent: Math.round(resultPercent) + '%', flight_time: resD,
